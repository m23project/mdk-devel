...
# MUST/WILL be set (default "Xorg").
if [ -z "$XSERVER" ]; then
 NEWXSERVER="$(getbootparam xserver 2>/dev/null)"
 [ -z "$NEWXSERVER" ] && NEWXSERVER="Xorg"
 XSERVER="$NEWXSERVER"
 echo "XSERVER='$XSERVER'" >> /etc/sysconfig/knoppix ; echo "XSERVER='$XSERVER'" >> /etc/sysconfig/xserver
fi

# MUST/WILL be set (default "vesa").
if [ -z "$XMODULE" ]; then
 NEWXMODULE="$(getbootparam xmodule 2>/dev/null)"
 [ -z "$NEWXMODULE" ] && NEWXMODULE="vesa"
 XMODULE="$NEWXMODULE"
 echo "XMODULE='$XMODULE'" >> /etc/sysconfig/knoppix
...
if [ -z "$NOMODELINES" ]; then
MONITOR="$(ddcxinfo-knoppix -monitor)"
else
MONITOR="$(ddcxinfo-knoppix -monitor | sed 's/ModeLine/# ModeLine/ig')"
fi
RC="$?"
...
# AlpsPS/2 | SynPS/2 Touchpad
TOUCHPAD=""
if [ -z "$MOUSEFULLNAME" ]; then
  case "AlpsPS/2" in *$MOUSEFULLNAME*)
    TOUCHPAD='s|#       InputDevice    "AlpsPad"|        InputDevice    "AlpsPad"|g'
  ;; esac
  case "SynPS/2" in *$MOUSEFULLNAME*)
    TOUCHPAD='s|#       InputDevice    "SynPad"|        InputDevice    "SynPad"|g'
  ;; esac
fi
...
# /etc/X11/XF86Config is the configuration file for VERY OLD cards that still need XFree3.
if [ -r /etc/X11//etc/X11/XF86Config.in ] && [ ! -f /etc/X11/XF86Config -o /etc/X11/XF86Config.in -nt /etc/X11/XF86Config ]; then 
...
# (Re-)create /etc/X11/xorg.conf if not present or /etc/X11/xorg.conf.in has been updated recently
if [ -r /etc/X11/xorg.conf.in ] && [ ! -f /etc/X11/xorg.conf -o /etc/X11/xorg.conf.in -nt /etc/X11/xorg.conf ]; then 
rm -f /etc/X11/xorg.conf /etc/X11/xorg.conf.new 2>/dev/null
...
SWCURSOR=""
case "$XMODULE" in trident) SWCURSOR='s|^.*#Option.*"sw_cursor".*$|Option "sw_cursor"|g;';; esac
