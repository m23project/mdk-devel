...
**/
function HELPER_calcScancodes($in)
{
		/**
		**name isUpper($char)
	...
	for ($i = 0; $i < strlen($in); $i++)
	{
		...
		if ('°' == $char)
		{
			...
			$scanCodeSequence .= getKey($special).' ';
		}
		else
			$scanCodeSequence .= getKey($char).' ';
	}
...
print(HELPER_calcScancodes($argv[1]));
?>