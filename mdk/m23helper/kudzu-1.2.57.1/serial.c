...
struct device *serialProbe(enum deviceClass probeClass, int probeFlags,
	...
	if (!dir) return devlist;
	while ((entry = readdir(dir))) {
	    struct stat sbuf;
	    char lockfile[32];
	    if (strncmp(entry->d_name, "ttyS",4) && strncmp(entry->d_name,"ttyUSB",6)) continue;
	    if (strncmp(entry->d_name, "ttyS",4))
			x = atoi(entry->d_name+6);
	    else
			x = atoi(entry->d_name+4);

	    if (x==console || x==stdin_line) continue;
	    snprintf(port,20,"/dev/%s",entry->d_name);

	    /* Make sure it's not in use */
	    snprintf(lockfile,32,"/var/lock/LCK..%s",entry->d_name);
