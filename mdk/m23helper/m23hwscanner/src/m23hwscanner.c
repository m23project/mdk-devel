...
**description gets all cdrom drives and generates an append line for the
lilo.conf all cdrom are set to ide-scsi emulation
...
void addPartition(const char *path, const char *fs, const char *type, int bootable)
{
	...
	if ((void *)strstr(type,"extended") > NULL)
		return;

	if (((void *)strstr(fs,"ext3") > NULL) ||
		((void *)strstr(fs,"ext2") > NULL) ||
		((void *)strstr(fs,"reiserfs") > NULL))
		addLinux(path,fs,strcmp(path,rootDevice)==0);

	if (((void *)strstr(fs,"fat32") > NULL) ||
		((void *)strstr(fs,"fat16") > NULL) ||
		((void *)strstr(fs,"vfat") > NULL) ||
		((void *)strstr(fs,"ntfs") > NULL))
		addWindows(path,fs,bootable);
		
	if ((void *)strstr(fs,"linux-swap") > NULL)
					...
					if (strstr(currentDev->path,"md") == NULL)
						{
							...
							addAssoc(key, (char *)ped_partition_type_get_name (part->type));
						};
